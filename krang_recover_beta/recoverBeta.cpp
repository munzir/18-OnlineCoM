...
int convergeToBeta() {
    ...
    for (int i = 0; i < numBodies; i++) {
        ...
        nonIdealBetaParams(0, i * bodyParams + 0) = mi + perturbedValue;
        nonIdealBetaParams(0, i * bodyParams + 1) = xMi + perturbedValue;
        nonIdealBetaParams(0, i * bodyParams + 2) = yMi + perturbedValue;
        nonIdealBetaParams(0, i * bodyParams + 3) = zMi + perturbedValue;
    ...
    Eigen::MatrixXd phiVecTimesCurrBeta(1, 1);
    ...
    for (int pose = 0; pose < numInputPoses && hasConverged == 0; pose++) {
        ...
        phiVecTimesCurrBeta = phiVec * currBeta.transpose();
        // Error is phiVecTimesCurrBeta - xCOM since xCOM is zero error is also
        // just phiVecTimesCurrBeta
        // Use absolute value of error to see if solution is suitable or not
        if (abs(phiVecTimesCurrBeta(0, 0)) <= suitableError) {
            hasConverged = 1;
        }
        // Should/can also write the error to a file for analysis
        xCOMErrorFile << phiVecTimesCurrBeta << "\n";
